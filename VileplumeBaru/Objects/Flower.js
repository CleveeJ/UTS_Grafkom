...
export class Flower {
  ...
  constructor(GL, SHADER_PROGRAM, _position, _color) {
    ...
    // ====== Kelopak ======
    this.petalTop = new EllipticParaboloid(GL, SHADER_PROGRAM, _position, _color, 0.9, 0.65, 0.12, 48, redPetal);
    this.petalRim = new PetalRim(GL, SHADER_PROGRAM, _position, _color, 0.9, 0.65, 0.12, 0.05, redPetal);
    this.petalBottom = new EllipticParaboloid(GL, SHADER_PROGRAM, _position, _color, 0.9, 0.65, 0.12, 48, redPetal);
    ...
    for (let i = 0; i < PETAL_COUNT; i++) {
      ...
      for (let k = 0; k < spotCount; k++) {
        ...
        const circle = new BezierCircle(GL, SHADER_PROGRAM, _position, _color, radius, 20, white);
        const offset = [
  ...
  setup() {
    ...
    this.petalSpots.forEach(spots =>
      spots.forEach(s => s.circle.setup())
    );
  ...
  render(_MMatrix, PARENT_MATRIX, time = 0) {
    ...
    const tilt = Math.sin(time * 0.5) * 0.3;      // rotasi ringan
    const scale = 1.0 + Math.sin(time * 2.0) * 0.05; // efek "mekar"
    ...
    for (let i = 0; i < PETAL_COUNT; i++) {
      ...
      let M = LIBS.get_I4();
      LIBS.translateY(M, PETAL_Y_OFFSET);
      LIBS.translateZ(M, PETAL_Z_FLAT);
      LIBS.rotateX(M, Math.sin(time + i) * 0.1);
      LIBS.rotateZ(M, ang);
      LIBS.translateX(M, PETAL_RADIUS);
      LIBS.translateZ(M, PETAL_Z_FLAT);
      LIBS.rotateY(M, PETAL_TILT);

      // lapisan kelopak bawah
      let Mbottom = LIBS.clone(M);
      for (let z = 1; z <= 3; z++) {
