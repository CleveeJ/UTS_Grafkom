...
export class Pohon {
    ...
    constructor(GL, SHADER_PROGRAM, _position=[0,0,0], _color=[0.4,0.25,0.1], _normal=null, scale = 1) {
        this.GL = GL;
        ...
        LIBS.translateY(daun1.MOVE_MATRIX, 2 + 1.0 * scale); // di atas batang
        ...
        LIBS.translateY(daun2.MOVE_MATRIX, 2 + 0.5 * scale); // sedikit di bawah daun1

        // ===== DAUN 3 (paling bawah) =====
        const daun3 = new Kerucut(GL, SHADER_PROGRAM, _position, _color, _normal, 1.0, 1.4, 36, [0.1, 0.4, 0.1]);
        daun3.MOVE_MATRIX = LIBS.get_I4();
        LIBS.translateY(daun3.MOVE_MATRIX, 2); // di atas batang tapi paling bawah dari daun
